...
--result:
执行用时：172 ms, 在所有 C++ 提交中击败了70.67%的用户
内存消耗：52.8 MB, 在所有 C++ 提交中击败了53.33%的用户
通过测试用例：178 / 178
 */
...
public:
      ...
      for (int cat = 0; cat < total; cat++) {
        ...
        for (auto &dir : dirs) {
          ...
          for (int row = catRow + dir[0], col = catCol + dir[1], jump = 1;
               row >= 0 && row < rows && col >= 0 && col < cols &&
               grid[row][col] != '#' && jump <= catJump;
               row += dir[0], col += dir[1], jump++) {
            int nextMouse = getPos(mouseRow, mouseCol),
  ...
  int getPos(int row, int col) { return row * cols + col; }
};
...
int main() {
  ...
  IC(catJump, mouseJump);

  Solution *solution = new Solution();
  IC(solution->canMouseWin(grid, catJump, mouseJump) == 1);
