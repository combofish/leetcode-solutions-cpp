...
private:
    ...
    bool backtracking(vector<vector<string>> &tickets, string station, vector<bool> &used) {
        ...
        for (int i = 0; i < tickets.size(); i++) {
            if (tickets[i].front() == station && !used[i]) {
                used[i] = true;
                path.push_back(station);
                if (backtracking(tickets, tickets[i].back(), used)) return true;
                path.pop_back();
                used[i] = false;
            }
        }
